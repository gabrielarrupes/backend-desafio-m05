...
const postCharge = async (req, res) => {
  const { idcustomer, value, duedate, status, description } = req.body;
...
const putCharge = async (req, res) => {
  ...
  try {
    ...
    const chargeUpdate = await connection("charge")
      .where({ id })
      .update({
        idcustomer,
        value,
    ...
    return res
      .status(200)
      .json({ message: "Cobrança atualizada com sucesso!" });
  } catch (error) {
    return res.status(500).json("Erro interno do servidor");
  }
};
...
const deleteCharge = async (req, res) => {
  ...
  try {
    ...
    if (chargeId.status === true) {
      return res
        .status(403)
        .json({ message: "Não é permitido excluir uma cobrança paga!" });
    }
    ...
    if (chargeDate < currentDate) {
      return res
        .status(403)
        .json({ message: "Não é permitido excluir uma cobrança vencida!" });
    }
